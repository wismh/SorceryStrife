...
2. **крок-2** `chore`: `CLAUDE.md` + код-стиль + Zenject SerializeField/Inject контракт застосовані до існючого коду.
3. **крок-3** `refactor`: заміна reflection-баф-системи на явну `StatType`-модель (див. розділ 9) — потрібно **до** ECS-бою, бо reflection несумісний з Burst.
4. **крок-4** `feat`: Bootstrap/ProjectContextInstaller, GlobalGameStateMachine (заміна прямих `SceneManager.LoadScene`).
