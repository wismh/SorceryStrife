...
4. **крок-4** `feat`: Bootstrap/ProjectContextInstaller, GlobalGameStateMachine (заміна прямих `SceneManager.LoadScene`). ✅ зроблено (коміт `38d03dd`) — з однією свідомою зміною обсягу: замість справжнього Zenject `ProjectContext`-префаба (потребує ручного створення в Editor — GameObject + `ProjectContext`-компонент + `SceneContext` у сцені `MainMenu`, це я не можу безпечно зробити наосліп) — `Bootstrap` реалізовано через `[RuntimeInitializeOnLoadMethod(BeforeSceneLoad)]`, чистий C#, без сцен/префабів. `GlobalGameStateMachine` (`MenuState`/`GameplayState`) і `SceneInBuild`-константи — як і планувалось. Якщо/коли схочеш "справжній" ProjectContext — заміна локалізована в `Bootstrap.cs`, решта коду вже йде через `Bootstrap.StateMachine`, а не напряму `SceneManager.LoadScene`.
...
6. **крок-6** `feat`: підключення `com.unity.entities` + `com.unity.entities.graphics`, базові `IComponentData` + baking для `Entity`/`EntityCharacteristics` (паралельно з MonoBehaviour-версією, не видаляємо одразу).
