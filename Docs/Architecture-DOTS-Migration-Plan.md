...
Перейменування `MiniJam` → `_Project` — окремий, ізольований коміт (`ss-1 chore: restructure to _Project layout`), без логічних змін, щоб diff був чистим і рев'ювся окремо від решти.
...
Це дає основний виграш (пулінг+ECS для мас-контенту) без інвестиції в GPU skeletal-анімацію під ECS (Vertex Animation Textures / Kinemation-подібні рішення) — той шлях лишаю як **стретч-ціль ss-12 (perf-крок)**: якщо після профайлінгу бюджетованих компаньйонів `Animator` все ще є вузьким місцем, тоді розглядаємо VAT-бейкинг існуючих анімацій під `Entities Graphics`. Не інвестую в це одразу, бо це окремий пайплайн (потрібен бейкер анімацій, нових шейдерів) — недешево для джем-проєкту, поки немає доказу, що воно справді потрібне.
...
1. **ss-1** `chore`: реструктуризація папок під `_Project` (без зміни логіки).
2. **ss-2** `chore`: `CLAUDE.md` + код-стиль + Zenject SerializeField/Inject контракт застосовані до існючого коду.
3. **ss-3** `refactor`: заміна reflection-баф-системи на явну `StatType`-модель (див. розділ 9) — потрібно **до** ECS-бою, бо reflection несумісний з Burst.
4. **ss-4** `feat`: Bootstrap/ProjectContextInstaller, GlobalGameStateMachine (заміна прямих `SceneManager.LoadScene`).
5. **ss-5** `chore`: апгрейд Unity 2022.3.53f1 → Unity 6 LTS (окрема ізольована гілка, без інших змін одночасно).
6. **ss-6** `feat`: підключення `com.unity.entities` + `com.unity.entities.graphics`, базові `IComponentData` + baking для `Entity`/`EntityCharacteristics` (паралельно з MonoBehaviour-версією, не видаляємо одразу).
7. **ss-7** `feat`: конверсія снарядів в ECS + Entities Graphics рендер + Burst job на рух/колізію.
8. **ss-8** `feat`: конверсія ворогів (рух, melee/range атака, здоров'я/смерть) в ECS + бюджетований пул GameObject-компаньйонів для анімацій.
9. **ss-9** `feat`: `WaveSpawnSystem` — переніс `EnemySpawner` на ECB-спавн.
10. **ss-10** `feat`: `PickupMagnetSystem` для `Experience` + Entities Graphics рендер пікапів.
11. **ss-11** `refactor`: прибрати мертвий MonoBehaviour-код (`PoolOfObject<Enemy>`, старі контролери) після того, як ECS-шлях підтверджено стабільним.
12. **ss-12** `perf`: Burst/Job-профайлінг, `RunFlowStateMachine`, оцінка VAT-стретчу для компаньйонів, фінальний прохід по code style.

Кожен крок — окрема гілка `feature/ss-N-опис` від `main`, PR на review, мержиться послідовно (щоб можна було грати на кожному кроці — hybrid дозволяє це).

## 6. Unity-версія

**Підтверджено:** апгрейд `minijam` з **2022.3.53f1** на **Unity 6 LTS**, щоб зрівнятись із tactics-cards і Lumenwake template та отримати актуальний `com.unity.entities`/`com.unity.entities.graphics`. Робимо це окремим ізольованим кроком (**ss-5**) — тільки апгрейд, без інших змін в тому ж PR, щоб у разі проблем було легко звузити причину.
...
Це **останній великий крок** (після ss-1..ss-12, коли рефакторинг стабілізувався) — переписувати історію, яка ще рухається, означає робити це двічі.
...
Торкається: 4 `Item`-асети (переписати дані в новому форматі — числа ті самі), 4 `Caster`-класи (заміна виклику баф-лукапу), `PlayerInventory` (заміна словника/методу), `ItemCard`/`UpgradeCard` (заміна reflection-рендеру), видалення `Utils.GetAllListProperties`. Контрольований, ізольований рефакторинг — саме тому він тепер **ss-3**, одразу після код-стилю і **до** будь-якого ECS-бою (п. 5.5), а не як пізній cleanup.

## 9. Відкриті питання

Основні рішення прийняті. Лишається одне практичне:

1. **Хто фізично рухає файли для ss-1** (реструктуризація `Assets/MiniJam` → `Assets/_Project`, розбивка на `Entities/Gateways/Mechanics/UI`): я можу зробити це через `git mv` (пари asset+`.meta` рухаються разом, GUID-посилання в сценах/префабах не ламаються — Unity резолвить по GUID, не по шляху), але я не можу відкрити Unity Editor тут, щоб перевірити консоль на помилки після операції. Після мого `git mv`-проходу потрібно буде відкрити проєкт у Editor і перевірити, що нічого не зламалось (особливо `Build Settings` — порядок сцен там для `SceneManager.LoadScene(0/1)`). Ок робити так, чи хочеш сам перенести папки в Editor (drag&drop), а я підключусь на рівні коду/неймспейсів/інсталерів після?
