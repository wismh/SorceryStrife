...
- `MainMenu`'s Start button and `Player`'s death handler both `[Inject]` the state machine and call `_stateMachine.Enter<GameplayState>()` / `Enter<MenuState>()` (awaited via `async UniTaskVoid` + `.Forget()`) rather than reaching for a static or `new`-ing a state. `MenuState`/`GameplayState : StateBase` wrap `SceneManager.LoadScene(SceneInBuild.*)` in an `override UniTask Enter()`.
