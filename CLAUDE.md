...
- `BootstrapScene.unity` (Build Settings index 0) is content-free — a camera plus one `Bootstrap` GameObject (`Mechanics/GameBootstrap/Bootstrap.cs`) whose `Start()` does `SceneManager.LoadScene(SceneInBuild.MainMenu)` and nothing else, mirroring the template's `Bootstrap → MainScene` handoff. It has no `SceneContext`, so `ProjectContext` is not resolved yet at this point.
...
- `ProjectContextInstaller` (`Mechanics/GameBootstrap/ProjectContextInstaller.cs`) binds `GlobalGameStateMachine` as a project-wide singleton, seeded with `MenuState` (Build Settings index 1, `MainMenu.unity`) via `SetInitial` so landing in the menu scene (via `Bootstrap`) doesn't also reload it.
- Zenject instantiates `ProjectContext` lazily and `DontDestroyOnLoad`s it the first time any `SceneContext` resolves its parent container — both `MainMenu.unity` and `SampleScene.unity` already have a `SceneContext`, so `GlobalGameStateMachine` is `[Inject]`-able from either scene automatically (first resolved when `MainMenu` loads), no per-scene rebinding needed.
